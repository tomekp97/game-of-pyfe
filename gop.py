...
def check_alive_neighbours(array, index):
    ...
    if array[index].y != 0 and array[index].x != 0:
        n_top_left = array[index  - (CELL_INDEX_CHANGE + 1)]
        neighbours.append(n_top_left)

    if array[index].y != 0:
        n_top = array[index - CELL_INDEX_CHANGE]
        ...
        if array[index].x != WIDTH - CELL_INDEX_CHANGE:
            n_top_right = array[index - (CELL_INDEX_CHANGE - 1)]
    ...
    try:
        n_bottom_left = array[index + (CELL_INDEX_CHANGE - 1)]
        if n_bottom_left.x != 0 and n_bottom_left.y != HEIGHT - CELL_INDEX_CHANGE:
            neighbours.append(n_bottom_left)
    except:
        pass

    try:
        n_bottom = array[index + CELL_INDEX_CHANGE]
        if n_bottom.y != HEIGHT - CELL_INDEX_CHANGE:
            neighbours.append(n_bottom)
    except:
        pass

    try:
        n_bottom_right = array[index + (CELL_INDEX_CHANGE + 1)]
        if n_bottom_right.x != WIDTH - CELL_INDEX_CHANGE and n_bottom_right.y != HEIGHT - CELL_INDEX_CHANGE:
